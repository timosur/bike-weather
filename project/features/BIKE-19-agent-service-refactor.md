...
|---|-----------|--------|-------|
| AC-1 | `publisher.py` deleted, no imports | PASS | File removed, grep confirms zero references |
| AC-2 | `__main__.py` starts FastAPI server | PASS | Runs `uvicorn.run("agent.server:app", ...)` |
| AC-3 | CLI code removed from `main.py` | PASS | No argparse, Rich, console.input, _print_result, _cli_progress, _setup_logging, main(), run() |
| AC-4 | `run_category()` and `run_urls()` return `list[ProductData]` | PASS | Signature verified, no publishing step |
| AC-5 | `config.py` has no `admin_api_url/token/dev_email` | PASS | grep confirms zero references |
| AC-6 | `CATEGORY_ZONE_MAP` in backend | PASS | Exists in `backend/app/api/routes/admin/products.py` with all 18 categories |
| AC-7 | Dockerfile uses `uvicorn` entrypoint | PASS | `ENTRYPOINT ["uvicorn", "agent.server:app", "--host", "0.0.0.0", "--port", "8001"]` |
| AC-8 | `rich` removed from `pyproject.toml` | PASS | grep confirms zero references |
| AC-9 | All HTTP endpoints work | PASS | Tested: /health, /shops, /categories, GET /jobs, POST /jobs, POST /jobs/urls, GET /jobs/{id} |
| AC-10 | Backend proxy routes unchanged | PASS | Code review confirms all proxy routes use agent HTTP — no change needed |
| AC-11 | Approval flow works | PASS | Code review: `approve_agent_import` sends `BulkProductItem` list to `_bulk_import()` directly |
| AC-12 | Agent tests pass | PASS | 32 passed, 1 pre-existing failure in `test_scraper` (unrelated mock bug) |
| AC-13 | `run_category()` returns products only | PASS | No `publish_products` or `publish_with_review` calls remain |

### Edge Cases

| # | Case | Status | Notes |
|---|------|--------|-------|
| EC-1 | No LLM API key | PASS | `_run_job()` catches all exceptions → job status set to FAILED with error message |
| EC-2 | `python -m agent` starts server | PASS | Confirmed: starts uvicorn on port 8001 |
| EC-3 | Expired/missing job → 404 | PASS | Tested: `GET /jobs/nonexistent-id` returns HTTP 404 |
| EC-4 | Unknown category ID in backend | PASS | Backend uses `CATEGORY_ZONE.get(cat.id, "other")` — defaults to `"other"` |
| EC-5 | No `rich`/`console.print` remaining | PASS | grep confirms zero `from rich` or `console.print` in agent code |

### Security Audit

| Check | Status | Notes |
|-------|--------|-------|
| Auth bypass | PASS | Agent is internal (no auth by design); all backend proxy routes require `require_admin` |
| Authorization | PASS | Backend agent proxy: all 8 endpoints use `Depends(require_admin)` |
| Input injection | N/A | No new user input surfaces added; existing validation unchanged |
| Rate limiting | N/A | Agent endpoints are internal, proxied through rate-limited backend |
| Data exposure | PASS | No sensitive fields in agent responses; no credentials in config |
| Secrets | PASS | API keys loaded from env vars, not hardcoded. Stale `AGENT_ADMIN_DEV_EMAIL` in `.env` (low, cleanup) |

### Bugs Found

| # | Severity | Description | Steps to Reproduce | Priority |
|---|----------|-------------|-------------------|----------|
| 1 | Low | Stale `AGENT_ADMIN_DEV_EMAIL=admin@bike-weather.local` in `.env` | Open `.env`, line 13 — config field was removed but env var remains | Low — harmless (ignored by pydantic `extra="ignore"`), clean up at convenience |

### Verdict

**Production-ready: YES**

No blocking issues. 13/13 AC pass, 5/5 EC pass, security audit clean. One low-severity cleanup item (stale env var).
