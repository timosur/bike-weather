...
test.describe("Ride Report", () => {
  ...
  test("save changes button appears after editing a saved route", async ({ page }) => {
    ...
    // Wait for the edit form to fully load (edit mode re-mounts the form with initial values)
    await expect(page.getByText(/pre-filled|vorausgefüllt/i)).toBeVisible({ timeout: 10000 });

    // Dismiss onboarding if visible, to simplify the form DOM
    const gotItBtn = page.getByRole("button", { name: /got it/i }).first();
    if (await gotItBtn.isVisible({ timeout: 1000 }).catch(() => false)) {
      await gotItBtn.click();
    }

    // Open advanced options to access the distance field
    ...
    // Submit form (direct dispatch — Playwright click doesn't trigger form submit
    // in edit mode due to the nested button hydration issue in the DOM)
    await page.evaluate(() => {
