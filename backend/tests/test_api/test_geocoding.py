...
async def test_search_endpoint_returns_suggestions(
    mock_service: AsyncMock, authenticated_client: AsyncClient
) -> None:
    mock_service.search = AsyncMock(return_value=MOCK_SEARCH_RESULTS)

    response = await authenticated_client.get(
        "/api/geocoding/search", params={"q": "Berlin"}
    )
...
async def test_search_endpoint_missing_query_returns_422(
    ...
    response = await authenticated_client.get("/api/geocoding/search")
...
async def test_reverse_endpoint_missing_params_returns_422(
    authenticated_client: AsyncClient,
) -> None:
    response = await authenticated_client.get(
        "/api/geocoding/reverse", params={"lat": "52.5"}
    )
    assert response.status_code == 422


async def test_search_endpoint_unauthenticated_returns_401(
