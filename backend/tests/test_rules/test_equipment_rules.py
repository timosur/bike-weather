...
def test_long_ride_includes_repair_kit() -> None:
    ...
    items = get_equipment_items(weather, distance_km=40)
...
def test_evening_ride_includes_lights() -> None:
    ...
    items = get_equipment_items(weather, distance_km=20, ride_start_time="16:00")
    names = [i["name"] for i in items]
...
def test_always_includes_water() -> None:
    weather = _make_weather()
    items = get_equipment_items(weather, distance_km=10)
    names = [i["name"] for i in items]
...
def test_very_long_ride_includes_energy() -> None:
    weather = _make_weather()
    items = get_equipment_items(weather, distance_km=60)
    names = [i["name"] for i in items]
