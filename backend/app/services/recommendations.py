...
INTENSITY_LABELS = {"gemuetlich": "Relaxed", "moderat": "Moderate", "sportlich": "Sporty"}
...
def _clothing_dicts_to_schemas(items: list[dict]) -> list[ClothingItemSchema]:
    ...
    for item in items:
        alts = [
            ClothingAlternativeSchema(**a) for a in item.get("alternatives", [])
        ]
        result.append(ClothingItemSchema(
            id=item["id"],
            name=item["name"],
            icon=item["icon"],
            reason=item["reason"],
...
async def build_report(
    ...
    if lat is None or lon is None:
        raise ValueError("Location must have lat/lon coordinates")
    ...
    for day_idx, (location_name, day_lat, day_lon, date_str) in enumerate(day_locations):
        forecast = await ws.fetch_forecast(day_lat, day_lon, date_str)
        condition = compute_condition(forecast)
        conditions.append(condition)

        clothing = get_clothing_items(forecast, ride_input.bikeType, ride_input.intensity)
        ...
        day_forecasts.append(DayForecastSchema(
